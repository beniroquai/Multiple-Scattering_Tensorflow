...
class MuScatModel(object):
    ...
    def computesys(self, obj, is_zernike=False, is_padding=False, is_tomo = False, dropout_prob=1):
        ...
        if(is_padding):
            ...
            self.Nx_new=self.Nx*2
            self.Ny_new=self.Ny*2
            self.mysize_new=np.array((self.Nz, self.Nx_new, self.Ny_new))
            self.obj = obj
            self.dx=self.dx
            self.dy=self.dy
        else:
        ...
        if(is_padding): # we have to compute the dphi on a finer grid
            self.kxysqr_new = (tf_helper.abssqr(tf_helper.xx((self.mysize_new[1], self.mysize_new[2]), 'freq') / self.dx) + 
                               tf_helper.abssqr(tf_helper.yy((self.mysize_new[1], self.mysize_new[2]), 'freq') / self.dy)) + 0j;
            ## Forward propagator  (Ewald sphere based) DO NOT USE NORMALIZED COORDINATES HERE
            self.kzsqr_new = tf_helper.abssqr(self.k0) - self.kxysqr_new;
            self.kz_new=np.sqrt(self.kzsqr_new);
            self.kz_new[self.kzsqr_new < 0]=0;
            self.dphi = 2*np.pi*self.kz_new*self.dz;  # exp(1i*kz*dz) would be the propagator for one slice
    ...
    def computemodel(self):
        ...
        with tf.name_scope('Fwd_Propagate'):
            ...
            for pz in range(0, self.mysize[0]+1):
                ...
                with tf.name_scope('Refract'):
                    ...
                    #plt.imshow(np.squeeze(np.angle(sess.run(self.TF_A_prop[50,:,:])))), plt.colorbar(), plt.show()

                with tf.name_scope('Propagate'):
                    if(self.is_padding):
                        tf_paddings = tf.constant([[0,0], [self.mysize[1]//2, self.mysize[1]//2], [self.mysize[2]//2, self.mysize[2]//2]])
                        self.TF_A_prop = tf.pad(self.TF_A_prop, tf_paddings, mode='CONSTANT', name='TF_obj_pad')
                        #plt.imshow(np.squeeze(np.angle(sess.run(self.TF_A_prop[1,:,:])))), plt.colorbar(), plt.show()
                        #plt.imshow(np.squeeze(np.angle(sess.run(self.TF_myprop[1,:,:])))), plt.colorbar(), plt.show()
                        #plt.imshow(np.squeeze(np.abs(sess.run(tf.fft2d(self.TF_A_prop)[1,:,:]))**.1)), plt.colorbar(), plt.show()                        
                        self.TF_A_prop = tf.ifft2d(tf.fft2d(self.TF_A_prop) * self.TF_myprop) # diffraction step
                        self.TF_A_prop = self.TF_A_prop[:,self.Nx-self.Nx//2:self.Nx+self.Nx//2, self.Ny-self.Ny//2:self.Ny+self.Ny//2]
                    else:
                        #plt.imshow(np.squeeze(np.angle(sess.run(self.TF_A_prop[1,:,:])))), plt.colorbar(), plt.show()
                        #plt.imshow(np.squeeze(np.angle(sess.run(self.TF_myprop[1,:,:])))), plt.colorbar(), plt.show()
                        #plt.imshow(np.squeeze(np.abs(sess.run(tf.fft2d(self.TF_A_prop)[1,:,:]))**.1)), plt.colorbar(), plt.show()
            ...
            for pillu in range(0, self.Nc):
                with tf.name_scope('Back_Propagate_Step'):
                    with tf.name_scope('Adjust'):
                        ...
                        if(self.is_padding):
                            tf_paddings = tf.constant([[0,0], [self.mysize[1]//2, self.mysize[1]//2], [self.mysize[2]//2, self.mysize[2]//2]])
                            OneAmp = tf.pad(OneAmp, tf_paddings, mode='CONSTANT', name='TF_obj_pad')
                            self.TF_allAmp = tf.ifft2d(tf.fft2d(OneAmp) * tf.pad(self.TF_myAllSlicePropagator, tf_paddings, mode='CONSTANT', name='TF_obj_pad')) 
                            #plt.imshow(np.squeeze(np.angle(sess.run(self.TF_A_prop[1,:,:])))), plt.colorbar(), plt.show()
                            #plt.imshow(np.squeeze(np.angle(sess.run(self.TF_myprop[1,:,:])))), plt.colorbar(), plt.show()
                            #plt.imshow(np.squeeze(np.abs(sess.run(tf.fft2d(self.TF_A_prop)[1,:,:]))**.1)), plt.colorbar(), plt.show()                        
                            self.TF_allAmp = self.TF_allAmp[:,self.Nx-self.Nx//2:self.Nx+self.Nx//2, self.Ny-self.Ny//2:self.Ny+self.Ny//2] * self.TF_AdjustKZ * self.TF_AdjustKXY 
