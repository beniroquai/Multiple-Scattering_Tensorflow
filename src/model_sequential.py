        ...
        with tf.name_scope('Fwd_Propagate'):
            ...
            for pillu in range(0, self.Nc):
                ...
                for pz in range(0, self.mysize[0]):
                    ...
                    self.TF_f = tf.exp(self.TF_RefrEffect[pillu,:,:]*tf.complex(TF_real, TF_imag))
                    ...
                    with tf.name_scope('Propagate'):
                        TF_A_prop_z_tmp = tf.ifft2d(tf.fft2d(TF_A_prop_z_tmp) * self.TF_myprop[pillu,:,:]) # diffraction step
            ...
            for pillu in range(0, self.Nc):
                ...
                for pz in range(0, self.Nz):
                    with tf.name_scope('Back_Propagate_Step'):
                        TF_allAmp_list.append(tf.squeeze(tf.ifft2d(tf.fft2d(OneAmp) * self.TF_myAllSlicePropagator[pz,:,:])) * self.TF_AdjustKZ[pz,:,:] * self.TF_AdjustKXY[:,:])  # * (TF_AdjustKZ);  # 2x bfxfun.  Propagates a single amplitude pattern back to the whole stack
                ...
                with tf.name_scope('Adjust_Global_Phase'):
                    self.TF_allAmp_3dft = tf.fft3d(tf.expand_dims(TF_allAmp, axis=0))
                    ...
                    TF_allAmp = TF_allAmp * tf.exp(-1j * tf_global_phase);  # Global Phases need to be adjusted at this step!  Use the zero frequency
                ...
                with tf.name_scope('Sum_Amps'): # Normalize amplitude by condenser intensity
                    self.TF_allSumAmp += TF_allAmp;  # Superpose the Amplitudes
