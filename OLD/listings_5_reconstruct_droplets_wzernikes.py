...
my_res = sess.run(muscat.TF_obj)
plt.imshow(np.abs(my_res[:,muscat.mysize[1]//2,:]))
#%%
''' Optimize the model '''
...
for iterx in range(1,Niter):
    ...
    if(not np.mod(iterx, Ndisplay)):
        my_opt, my_res, my_loss, my_fidelity, my_negloss, my_posloss, my_tvloss, my_zernfactors =  \
            sess.run([tf_lossop, muscat.TF_obj, tf_loss, tf_fidelity, tf_negloss, tf_posloss, tf_tvloss, muscat.TF_zernikefactors], feed_dict={tf_meas:np_meas})
