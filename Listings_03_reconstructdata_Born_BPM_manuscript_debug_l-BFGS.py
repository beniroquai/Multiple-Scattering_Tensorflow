...
#%%        
        train_step = tf.contrib.opt.ScipyOptimizerInterface(
                tf_loss,
                method='L-BFGS-B',
                options={'maxiter': 10}, var_list = [muscat.TF_obj])

        sess.run(tf.global_variables_initializer())
        train_step.minimize(sess, feed_dict={muscat.tf_meas:np_meas, muscat.tf_learningrate:experiments.my_learningrate, muscat.tf_lambda_reg:experiments.lambda_reg, muscat.tf_eps:experiments.myepstvval})
        ...
        print('Loss@'+str(0)+': ' + str(my_loss) + ' - Fid: '+str(my_fidelity)+', Neg: '+str(my_negloss)+', TV: '+str(my_tvloss))
