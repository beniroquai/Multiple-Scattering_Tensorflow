...
subsample_fac_xy=1
sumbsampleZ=1
roisize = np.array((50, 50, 80))
mycenter_roi = np.array((allAmp_clean.shape[0]//2, 340, 290))

if(0):
    roisize = np.array((300, 300, 80))
    mycenter_roi = np.array((allAmp_clean.shape[0]//2, 300, 300))

mysize_new = allAmp_clean.shape
allAmp_clean_extract = nip.extract(allAmp_clean, roisize, mycenter_roi)
...
# show butterfly 
allAmpFT = nip.ft((nip.DampEdge(allAmp_clean_extract, rwidth=.25, axes=3)))
allAmpFT[:,allAmpFT.shape[2]//2,:]**.1
allAmpFT[:,:,allAmpFT.shape[2]//2]**.1
