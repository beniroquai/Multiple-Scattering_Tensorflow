...
for iroi = 1:nroi
    ...
    save(['S19_subroi' num2str(iroi)], 'allAmp_red', '-v7.3')
    
    subplot(2,nroi,iroi)
    imagesc(transpose(squeeze(abs(allAmp_red(:,25,:)))))
    colorbar, axis image, colormap gray
    title(['ABS#' num2str(iroi)])
    subplot(2,nroi,iroi+nroi)
    imagesc(transpose(squeeze(angle(allAmp_red(:,25,:)))))
    colorbar, axis image, colormap gray
