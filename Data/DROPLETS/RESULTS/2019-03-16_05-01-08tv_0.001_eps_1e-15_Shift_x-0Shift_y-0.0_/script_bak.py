...
plt.imshow(np.real(matlab_val[:,:,50]))
roisize=32
roicenter = np.array((150,125))
...
obj = np.ones(muscat.mysize)+muscat.nEmbb + 1j*np.zeros(muscat.mysize)
muscat.computesys(obj=obj, is_padding=is_padding, mysubsamplingIC=mysubsamplingIC, is_compute_psf='corr')

''' Create Model Instance'''
muscat.computemodel()
   
print('Starting Session and initialize all varialbes')
sess = tf.Session()
