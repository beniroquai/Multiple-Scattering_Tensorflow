...
if is_recomputemodel:
    ...
    if not is_estimatepsf:
      ...
      tf_lossop = tf_optimizer.minimize(tf_loss, var_list = [muscat.TF_obj,muscat.TF_obj_absorption, muscat.TF_shiftIcX, muscat.TF_shiftIcY, muscat.TF_zernikefactors])
        ...
        TF_zernloss = .02*reg.Reg_L2(muscat.TF_zernikefactors)
        tf_lossop_zern = tf_optimizer.minimize(TF_zernloss)
...
for iterx in range(iter_last,Niter):
    ...
    if is_estimatepsf:
        zern_error,_ = sess.run([TF_zernloss,tf_lossop_zern], feed_dict={muscat.tf_meas:np_meas, muscat.tf_learningrate:my_learningrate, muscat.tf_lambda_tv:lambda_tv, muscat.tf_eps:myepstvval})
        print(zern_error)
