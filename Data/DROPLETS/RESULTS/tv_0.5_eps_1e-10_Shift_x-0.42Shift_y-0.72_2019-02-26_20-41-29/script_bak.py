...
shiftIcY= .72 # has influence on the YZ-Plot - negative values shifts the input wave (coming from 0..end) to the left
...
obj = (((squeezefac*tf_helper.xx(mysize=obj.shape)**2)+(tf_helper.yy(mysize=obj.shape)**2)+(squeezefac*tf_helper.zz(mysize=obj.shape)**2))<(5**2))*dn#tf_go.generateObject(mysize=obj.shape, obj_dim=muscat.dx, obj_type ='sphere', diameter = mydiameter, dn = dn)
